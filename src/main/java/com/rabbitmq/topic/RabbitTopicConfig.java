...
public class RabbitTopicConfig {
	final static String MESSAGE = "xiaoxiong.message";
	final static String MESSAGES = "xiaoxiong.message.s";
	final static String YMQ = "xiaoxiong.ymq";
	@Bean
	...
	Binding bindingExchangeMessage(Queue queueMessage, TopicExchange topicExchange) {
		return BindingBuilder.bind(queueMessage).to(topicExchange).with("xiaoxiong.message");
		}
	...
	Binding bindingExchangeMessages(Queue queueMessages, TopicExchange topicExchange) {
		return BindingBuilder.bind(queueMessages).to(topicExchange).with("xiaoxiong.message.s");
		} 
	...
	Binding bindingExchangeYmq(Queue queueYmq, TopicExchange topicExchange) {
		return BindingBuilder.bind(queueYmq).to(topicExchange).with("xiaoxiong.#");
		}
}
